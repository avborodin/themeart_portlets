    ...
    <div id="{$instance->getUid()}" {$instance->getAttributeString()} class="opc-Video {$instance->getStyleClasses()}">
        ...
        {else}
            <div{if $instance->getProperty('video-responsive')} class="embed-responsive embed-responsive-16by9"{/if}>
                <video {if $instance->getProperty('thumbnail')}poster="/media/image/storage/opc/{$instance->getProperty('thumbnail')}"{/if} playsinline {if $instance->getProperty('video-width')}width="{if $instance->getProperty('video-responsive')}100%{else}{$instance->getProperty('video-width')}{/if}"{/if}
                       {if $instance->getProperty('video-height') && !$instance->getProperty('video-responsive')}height="{$instance->getProperty('video-height')}"{/if}
