{if $isPreview}
    ...
    {/if}
    <div class="text-center opc-ImageSlider" style="{$instance->getStyleString()}">
        <div>
            {file_get_contents($portlet->getBasePath()|cat:'icon.svg')}
            <span>{__('Marken-Slider')}</span>
        </div>
    </div>
{else}
    {$uid = $instance->getUid()}

    <div style="{$instance->getStyleString()}" class="slider-wrapper {$instance->getStyleClasses()}">
